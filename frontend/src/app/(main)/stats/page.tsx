  ...
  if (loading) {
    return (
      <div>
        ...
        <h1 className="font-black mb-8 lg:mb-12">数据概览</h1>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-4 mb-8 lg:mb-12">
        <div className="s-card">
          ...
          <p className="text-2xl md:text-4xl font-black">
            ...
            <span className="font-mono text-xs md:text-sm font-normal text-gray-500">
              /{stats?.total_tasks ?? 0}
            </span>
          </p>
        </div>
        <div className="s-card">
          ...
          <p className="text-2xl md:text-4xl font-black">
            ...
            <span className="font-mono text-xs md:text-sm font-normal text-gray-500"> 分钟</span>
          </p>
        </div>
        <div className="s-card">
          <p className="s-label">新增词汇</p>
          <p className="text-2xl md:text-4xl font-black">{stats?.new_vocab_count ?? 0}</p>
        </div>
      </div>

      <div className="grid grid-cols-2 md:grid-cols-2 gap-3 md:gap-4 mb-8 lg:mb-12">
        <div className="s-card">
          <p className="s-label">复习次数</p>
          <p className="text-2xl md:text-4xl font-black">{stats?.review_count ?? 0}</p>
        </div>
        <div className="s-card">
          <p className="s-label">复习完成率</p>
          <p className="text-2xl md:text-4xl font-black">
            {Math.round((stats?.review_completion_rate ?? 0) * 100)}
            <span className="font-mono text-xs md:text-sm font-normal text-gray-500">%</span>
          </p>
        </div>
      </div>
          ...
          <div className="border-2 border-black min-w-[360px]">
            ...
            {stats?.daily_breakdown?.map((day: any) => (
              ...
              >
                ...
                <span className="font-mono text-xs md:text-sm text-right">{day.new_vocab}</span>
              </div>
            ))}
          </div>
        </div>
